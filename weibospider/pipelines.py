...
class WeibospiderPipeline:
    ...
    def process_item(self, item, spider):
        ...
        if isinstance(item, weibospider.items.OriginWeiboItem):
            ...
            data = (item['user_id'], item['friends_count'], item['followers_count'], item['statuses_count'])
        elif isinstance(item, weibospider.items.WeiboCityItem):
            sql2 = 'select * from weibo_city where origin_weibo_id = %s'
            data2 = item['origin_weibo_id']
            sql3 = 'update weibo_city set  origin_user_id=%s, origin_weibo_content=%s,publish_time=%s,repost_count=%s,like_count =%s where origin_user_id = %s'
            data3 = (item['origin_user_id'], item['origin_weibo_content'], item['publish_time'],
                     item['repost_count'], item['like_count'], item['origin_weibo_id'])
            sql = 'INSERT INTO weibo_city(origin_weibo_id, origin_user_id, origin_weibo_content,publish_time,repost_count,like_count)VALUES(%s,%s,%s,%s,%s,%s) '
            data = (item['origin_weibo_id'], item['origin_user_id'], item['origin_weibo_content'], item['publish_time'],
                    item['repost_count'], item['like_count'])
        elif isinstance(item, weibospider.items.CityRepostWeiboItem):
            sql2 = 'select * from repost_weibo_city where repost_weibo_id = %s'
            data2 = item['repost_weibo_id']
            sql3 = 'update repost_weibo_city set origin_weibo_id = %s,origin_user_id=%s, repost_user_id=%s,repost_weibo_content=%s,repost_publish_time=%s where repost_weibo_id = %s'
            data3 = (item['origin_weibo_id'], item['origin_user_id'], item['repost_user_id'],
                     item['repost_weibo_content'], item['repost_publish_time'], item['repost_weibo_id'])
            sql = 'INSERT INTO repost_weibo_city(origin_weibo_id, origin_user_id, repost_weibo_id,repost_user_id,repost_weibo_content,repost_publish_time)VALUES(%s,%s,%s,%s,%s,%s) '
            data = (item['origin_weibo_id'], item['origin_user_id'], item['repost_weibo_id'], item['repost_user_id'],
                    item['repost_weibo_content'], item['repost_publish_time'])
        elif isinstance(item, weibospider.items.CityUserInfoItem):
            sql2 = 'select * from user_info_city where user_id = %s'
            data2 = item['user_id']
            sql3 = 'update user_info_city set friends_count=%s, followers_count=%s,statuses_count =%s where user_id = %s'
            data3 = (item['friends_count'], item['followers_count'], item['statuses_count'], item['user_id'])
            sql = 'INSERT INTO user_info_city(user_id, friends_count, followers_count,statuses_count)VALUES(%s,%s,%s,%s) '
            data = (item['user_id'], item['friends_count'], item['followers_count'], item['statuses_count'])    
